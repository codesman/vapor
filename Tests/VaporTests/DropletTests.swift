...
class DropletTests: XCTestCase {
    static let allTests = [
        ...
        ("testHeadResponse", testHeadResponse),
    ]
    ...
    func testHeadResponse() throws {
        ...
        drop.console.wait(seconds: 2)
        let res = try drop.client.request(.head, "http://0.0.0.0:8080/foo", query: [:])

        let body = try res.bodyString()
        let length = res.headers["Content-Length"]?.int
